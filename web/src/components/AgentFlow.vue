  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ allDone ? '分析完成' : 'Agent 并发执行中...' }}</h2>
      </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
      ...
      >
        <div class="flex items-center justify-between mb-1">
          <span class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ agent.icon }} {{ shortName(agent.name) }}</span>
          <StatusBadge :agent="agent" />
        </div>
        <div v-if="agent.done && agent.output" class="text-xs text-gray-600 dark:text-gray-400 mt-1 truncate" :title="agent.output">
          {{ agent.output }}
        </div>
        ...
        <div v-if="agent.qualityScore" class="mt-1.5 group/score relative">
          ...
          <!-- Judge 评语 tooltip -->
          <div v-if="agent.judgeComment"
            class="absolute bottom-full left-0 mb-2 w-64 p-2.5 rounded-lg text-xs leading-relaxed
              bg-gray-900 dark:bg-gray-700 text-gray-100 dark:text-gray-200
              shadow-lg opacity-0 invisible group-hover/score:opacity-100 group-hover/score:visible
...
function buildChartData() {
  ...
  agentKeys.forEach((key, i) => {
    ...
    nodes.push({
      ...
      itemStyle: {
        color: state === 'done'
          ? (isDark.value ? '#1a3a2a' : '#dcfce7')
          : (isDark.value ? '#334155' : '#f9fafb'),
        borderColor: state === 'done'
          ? (isDark.value ? '#22c55e' : '#86efac')
          : (isDark.value ? '#4b5563' : '#e5e7eb'),
        borderWidth: 1,
      },
      label: {
        show: true,
...
watch(() => props.agents.map(a => ({ done: a.done, output: a.output, qualityScore: a.qualityScore, judgeComment: a.judgeComment, duration: a.duration })),
  () => {
    updateChart()
    if (allDone.value) {
      stopPulse()
