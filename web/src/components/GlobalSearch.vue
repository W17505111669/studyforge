        ...
        >
          ...
          <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <input
              ...
              @input="onInput"
            />
            <kbd class="hidden sm:inline-flex items-center px-2 py-0.5 rounded border border-gray-300 dark:border-gray-600 text-xs text-gray-400 dark:text-gray-500 font-mono">
              Esc
            </kbd>
          </div>
          ...
          <div class="max-h-[50vh] overflow-y-auto">
            ...
            <div v-else-if="searched && results.length === 0" class="px-5 py-10 text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                未找到与「<span class="text-gray-700 dark:text-gray-300">{{ query }}</span>」相关的结果
              </p>
            </div>
            ...
            <template v-else-if="results.length > 0">
              <!-- 按类型分组 -->
              <template v-for="(group, gi) in groupedResults" :key="group.type">
                <div class="px-5 py-2 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider bg-gray-50 dark:bg-gray-800/50">
                  {{ group.label }}
                ...
                >
                  <!-- 类型图标 -->
                  <div :class="iconClass(item.type)" class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0">
                    <!-- 材料图标 -->
                    <svg v-if="item.type === 'material'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <!-- 卡片图标 -->
                    <svg v-else-if="item.type === 'card'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 3a1 1 0 000 2h6a1 1 0 100-2H7zM4 7a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zm-2 4a2 2 0 012-2h12a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4z"/>
                    </svg>
                    <!-- 题目图标 -->
          ...
          <!-- 底部快捷键提示 -->
          <div v-if="results.length > 0" class="px-5 py-2.5 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500">
            ...
            <span class="ml-auto">共 {{ results.length }} 条结果</span>
          </div>
        </div>
      </div>
    </Transition>
...
const flatResults = computed(() => {
  ...
  flat.forEach((item, i) => { item._flatIndex = i })
  return flat
})
