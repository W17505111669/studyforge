<template>
  <div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
    ...
    <div v-if="!loading && recommendations.length > 0"
      class="mb-6 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-950/40 dark:via-purple-950/30 dark:to-pink-950/20
        rounded-xl border border-indigo-100 dark:border-indigo-800/40 p-4 sm:p-5"
    >
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <router-link
          ...
          :to="rec.action_url"
          class="flex items-start gap-3 p-3 rounded-lg bg-white/70 dark:bg-gray-800/60
            hover:bg-white dark:hover:bg-gray-800 hover:shadow-sm transition-all
            border border-transparent hover:border-indigo-200 dark:hover:border-indigo-700/50
            group flex-1 min-w-0"
        >
          <span class="text-xl shrink-0 mt-0.5">{{ rec.icon }}</span>
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                {{ rec.title }}
              </span>
              <span v-if="rec.badge"
          ...
          <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 shrink-0 mt-1 group-hover:text-indigo-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <template v-if="loading">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div v-for="i in 4" :key="i" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 lg:col-span-2">
          <div class="skeleton skeleton-text w-32 h-5 mb-4"></div>
          <div class="skeleton skeleton-box w-full rounded-lg" style="height: 280px"></div>
        </div>
      </div>
      ...
      <!-- 材料列表骨架 -->
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-5">
        <div class="flex items-center justify-between mb-5">
          <div class="skeleton skeleton-text w-28 h-5"></div>
          <div class="skeleton skeleton-text w-20 h-4"></div>
        </div>
        <div class="space-y-3">
          <div v-for="i in 3" :key="i" class="rounded-lg border border-gray-100 dark:border-gray-700 md:flex md:items-center md:justify-between md:p-4">
    ...
    <template v-else>
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="hidden sm:flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500 mr-2">
                <span>活跃 <strong class="text-gray-700 dark:text-gray-300">{{ calendarStats.activeDays }}</strong> 天</span>
                <span>共 <strong class="text-gray-700 dark:text-gray-300">{{ calendarStats.totalActivities }}</strong> 次</span>
                <span v-if="calendarStats.streak > 0">连续 <strong class="text-emerald-600 dark:text-emerald-400">{{ calendarStats.streak }}</strong> 天</span>
              </div>
              <!-- 年份选择器 -->
              <select v-model.number="calendarYear"
                @change="loadCalendarData"
                class="text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700
                  text-gray-700 dark:text-gray-300 px-2 py-1 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
              >
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6">
          ...
          <div v-if="qualityDistTotal > 0" class="mb-3">
            ...
            <div class="flex h-2 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-700">
              <div class="bg-red-400 dark:bg-red-500 transition-all" :style="{ width: qualityDistPct('0-3') + '%' }"></div>
              <div class="bg-amber-400 dark:bg-amber-500 transition-all" :style="{ width: qualityDistPct('3-6') + '%' }"></div>
              <div class="bg-emerald-400 dark:bg-emerald-500 transition-all" :style="{ width: qualityDistPct('6-8') + '%' }"></div>
              <div class="bg-indigo-500 dark:bg-indigo-400 transition-all" :style="{ width: qualityDistPct('8-10') + '%' }"></div>
            </div>
            <div class="flex items-center gap-2 mt-1.5 text-[10px] text-gray-400 dark:text-gray-500">
              <span class="flex items-center gap-0.5"><span class="w-2 h-2 rounded-sm bg-red-400 dark:bg-red-500"></span>0-3</span>
              <span class="flex items-center gap-0.5"><span class="w-2 h-2 rounded-sm bg-amber-400 dark:bg-amber-500"></span>3-6</span>
              <span class="flex items-center gap-0.5"><span class="w-2 h-2 rounded-sm bg-emerald-400 dark:bg-emerald-500"></span>6-8</span>
              <span class="flex items-center gap-0.5"><span class="w-2 h-2 rounded-sm bg-indigo-500 dark:bg-indigo-400"></span>8-10</span>
            </div>
          </div>
          <!-- 最近 Judge 评语 -->
          <div v-if="judgeComments.length > 0" class="border-t border-gray-100 dark:border-gray-700 pt-3">
            ...
            <div class="space-y-2 max-h-36 overflow-y-auto custom-scroll">
              <div v-for="(c, i) in judgeComments" :key="i"
                class="text-xs p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600"
              >
                <div class="flex items-center gap-1.5 mb-0.5">
                  <span class="font-medium text-gray-700 dark:text-gray-300">{{ agentLabels[c.agent_name] || c.agent_name }}</span>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-5">
        ...
        <div v-else class="space-y-3">
          <div v-for="m in materials" :key="m.id"
            class="rounded-lg border border-gray-100 dark:border-gray-700 hover:border-primary-200 hover:bg-primary-50/30 dark:hover:bg-gray-700 transition-all cursor-pointer
              md:flex md:items-center md:justify-between md:p-4"
          >
            <!-- 移动端卡片布局 -->
            <div class="md:hidden p-4 pb-3">
              <router-link :to="`/materials/${m.id}`" class="font-medium text-gray-900 dark:text-gray-100 block truncate">{{ m.title }}</router-link>
            ...
            <span class="hidden md:inline-block px-3 py-1 rounded-full text-xs font-medium ml-4" :class="statusClass(m.status)">
              {{ statusLabel(m.status) }}
            </span>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-5">
          ...
          <div class="flex items-center gap-1">
            ...
            <div class="w-24 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500"
                :class="achievementUnlocked === achievementTotal && achievementTotal > 0 ? 'bg-amber-500' : 'bg-primary-500'"
        ...
        <div v-if="recentUnlocks.length > 0" class="mb-4 flex flex-wrap gap-2">
          <div v-for="a in recentUnlocks" :key="'recent-' + a.id"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          >
            <!-- 图标 -->
            <div class="text-2xl mb-1.5 transition-transform group-hover:scale-110" :class="{ 'grayscale opacity-50': !a.unlocked }">
              {{ a.icon }}
            </div>
            <!-- 名称 -->
            <div class="text-xs font-semibold mb-0.5 truncate"
            ...
            <div v-if="!a.unlocked" class="mt-1">
              <div class="w-full h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full bg-primary-400 rounded-full transition-all duration-300"
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="metric in metrics" :key="metric.label" class="text-center p-4 rounded-lg bg-gray-50 dark:bg-gray-700">
...
import { getUserStats, listMaterials, getMetrics, getAchievements, getRecommendations, getCalendarHeatmap } from '../api/client'
...
onMounted(async () => {
  try {
    const [statsRes, matRes, metricsRes, calendarRes, achieveRes, recRes] = await Promise.allSettled([
      getUserStats(),
      listMaterials(),
      getMetrics(),
