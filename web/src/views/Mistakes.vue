  ...
  <div class="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 gap-4">
      ...
      <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
        <!-- 错题重练按钮 -->
        <button
          v-if="stats.unreviewed > 0 && !retryMode && !consolidateMode"
          @click="startRetry"
        ...
        >
          ...
          错题重练 ({{ stats.unreviewed }})
        </button>
        <!-- 巩固强化按钮 -->
        <button
          v-if="stats.total > 0 && !retryMode && !consolidateMode"
    ...
    <div v-if="retryMode" class="space-y-6">
      ...
      <!-- 当前题目 -->
      <div v-if="retryQuizzes.length > 0 && retryIndex < retryQuizzes.length" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
        <div class="flex items-center gap-2 mb-4">
          <span class="px-2 py-0.5 rounded text-xs font-medium"
            :class="retryQuiz.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                    retryQuiz.difficulty === 'hard' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                    'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'">
            {{ retryQuiz.difficulty === 'easy' ? '简单' : retryQuiz.difficulty === 'hard' ? '困难' : '中等' }}
          </span>
          <span class="px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
            {{ typeLabel(retryQuiz.type) }}
          </span>
          <span v-if="currentMistakeItem" class="text-xs text-gray-400 dark:text-gray-500 ml-auto">
            上次错误答案：<span class="text-red-500 dark:text-red-400 font-medium">{{ currentMistakeItem.last_wrong_answer }}</span>
          </span>
        </div>
        ...
        <div v-if="retryQuiz.type === 'choice' && retryOptions.length" class="space-y-3 mb-6">
          ...
          >
            ...
            {{ opt }}
            <svg v-if="retryAnswered && String.fromCharCode(65 + idx) === retryQuiz.answer" class="inline w-4 h-4 ml-2 text-green-500" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div v-if="retryAnswered" class="space-y-4">
          <div class="p-4 rounded-lg" :class="retryIsCorrect ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800'">
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="font-medium" :class="retryIsCorrect ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'">
                {{ retryIsCorrect ? '回答正确！' : '回答错误' }}
              </span>
            </div>
            <div v-if="!retryIsCorrect" class="text-sm space-y-1">
              <p><span class="text-gray-500 dark:text-gray-400">你的答案：</span><span class="text-red-600 dark:text-red-400">{{ retrySelectedAnswer || retryTextAnswer }}</span></p>
              <p><span class="text-gray-500 dark:text-gray-400">正确答案：</span><span class="text-green-600 dark:text-green-400">{{ retryQuiz.answer }}</span></p>
            </div>
            <p v-if="retryQuiz.explanation" class="text-sm text-gray-600 dark:text-gray-400 mt-2">{{ retryQuiz.explanation }}</p>
          </div>
      ...
      <!-- 重练完成 -->
      <div v-if="retryIndex >= retryQuizzes.length" class="bg-white dark:bg-gray-800 rounded-xl border border-green-200 dark:border-green-800 p-6 text-center">
        ...
        <div class="flex justify-center gap-3">
          <button @click="exitRetry" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm rounded-lg transition">
            返回错题本
          </button>
        </div>
      </div>
    ...
    <div v-else-if="consolidateMode" class="space-y-6">
      <!-- 薄弱点分析 -->
      <div v-if="weakAreas.length > 0" class="bg-white dark:bg-gray-800 rounded-xl border border-violet-200 dark:border-violet-800 p-4 sm:p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            <p class="text-xs font-medium text-violet-700 dark:text-violet-300 truncate">{{ area.material_title || '未知材料' }}</p>
            <div class="flex items-center gap-2 mt-1 text-xs text-gray-600 dark:text-gray-400">
              ...
              <span v-if="area.main_type" class="px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{{ typeLabel(area.main_type) }}</span>
              <span v-if="area.main_difficulty" class="px-1.5 py-0.5 rounded"
                :class="area.main_difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                        area.main_difficulty === 'hard' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                        'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
        ...
        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
          <div
            ...
            :style="{ width: `${((consolidateIndex + (consolidateAnswered ? 1 : 0)) / consolidateQuizzes.length) * 100}%` }"
          ></div>
        </div>
      </div>

      <!-- 当前题目 -->
      <div v-if="consolidateQuizzes.length > 0 && consolidateIndex < consolidateQuizzes.length" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
        <div class="flex items-center gap-2 mb-4 flex-wrap">
          <span class="px-2 py-0.5 rounded text-xs font-medium"
            :class="cQuiz.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                    cQuiz.difficulty === 'hard' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                    'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'">
        ...
        <div v-if="cQuiz.type === 'choice' && cOptions.length" class="space-y-3 mb-6">
          ...
          >
            <span class="font-medium mr-2">{{ String.fromCharCode(65 + idx) }}.</span>
            {{ opt }}
            <svg v-if="consolidateAnswered && String.fromCharCode(65 + idx) === cQuiz.answer" class="inline w-4 h-4 ml-2 text-green-500" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
            </svg>
          </button>
        </div>
        ...
        <div v-if="consolidateAnswered" class="space-y-4">
          <div class="p-4 rounded-lg" :class="consolidateCorrect ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800'">
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="font-medium" :class="consolidateCorrect ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'">
                {{ consolidateCorrect ? '回答正确！' : '回答错误' }}
              </span>
            </div>
            <div v-if="!consolidateCorrect" class="text-sm space-y-1">
              <p><span class="text-gray-500 dark:text-gray-400">你的答案：</span><span class="text-red-600 dark:text-red-400">{{ consolidateAnswer || consolidateTextAnswer }}</span></p>
              <p><span class="text-gray-500 dark:text-gray-400">正确答案：</span><span class="text-green-600 dark:text-green-400">{{ cQuiz.answer }}</span></p>
            </div>
            <p v-if="cQuiz.explanation" class="text-sm text-gray-600 dark:text-gray-400 mt-2">{{ cQuiz.explanation }}</p>
          </div>
      ...
      <!-- 巩固完成 -->
      <div v-if="consolidateIndex >= consolidateQuizzes.length" class="bg-white dark:bg-gray-800 rounded-xl border border-violet-200 dark:border-violet-800 p-6 text-center">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-4">
          共 {{ consolidateQuizzes.length }} 题，正确 {{ consolidateCorrectCount }} 题，错误 {{ consolidateWrongCount }} 题
          <span v-if="consolidateQuizzes.length > 0" class="font-medium">
            ({{ Math.round((consolidateCorrectCount / consolidateQuizzes.length) * 100) }}%)
          </span>
        </p>
        <div class="flex justify-center gap-3">
          <button @click="exitConsolidate" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm rounded-lg transition">
            返回错题本
          </button>
        </div>
      </div>
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ stats.unreviewed }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">已复习</p>
          <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ stats.reviewed }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <label class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 cursor-pointer">
            ...
            />
            全选
            <span v-if="selectedIds.size > 0" class="text-primary-600 dark:text-primary-400">已选 {{ selectedIds.size }}</span>
          </label>
          <!-- 批量标记复习 -->
          <button
      ...
      <div v-if="loading" class="space-y-4">
        <div v-for="i in 3" :key="i" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 animate-pulse">
          ...
          <div class="h-5 bg-gray-200 dark:bg-gray-700 rounded w-2/3 mb-4"></div>
          <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded w-full mb-2"></div>
          <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded w-3/4"></div>
        </div>
      </div>

      <div v-else-if="filteredMistakes.length === 0" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
        <div class="w-16 h-16 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg class="w-8 h-8 text-green-500" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
          </svg>
        ...
        <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">
          {{ activeFilter === 'unreviewed' ? '没有待复习的错题' : activeFilter === 'reviewed' ? '还没有复习过任何错题' : '错题本是空的' }}
        </h3>
        <p class="text-gray-500 dark:text-gray-400 text-sm">
          {{ activeFilter === 'unreviewed' ? '继续保持，去做些练习吧！' : '去练习场答题，答错的题目会自动收集到这里' }}
        </p>
        <router-link to="/quiz" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm rounded-lg transition">
          去练习场
        </router-link>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        >
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 flex-wrap mb-2">
                  <span class="px-2 py-0.5 rounded text-xs font-medium"
                    :class="item.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                  ...
                  <span v-if="item.material_title" class="text-xs text-gray-400 dark:text-gray-500 truncate max-w-[200px]">
                    {{ item.material_title }}
                  </span>
                  <span v-if="item.reviewed" class="px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                    已复习
                  </span>
                ...
                <!-- 题目 -->
                <h3 class="text-sm sm:text-base font-medium text-gray-900 dark:text-gray-100 mb-3">{{ item.question }}</h3>
                ...
                <div v-if="item.quiz_type !== 'choice'" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  ...
                  <div class="px-3 py-2 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                ...
                <!-- 解析 -->
                <div v-if="item.explanation" class="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                  <span class="font-medium text-gray-700 dark:text-gray-300">解析：</span>{{ item.explanation }}
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- 操作栏 -->
          <div class="px-4 sm:px-5 py-3 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-2">
            <button
              v-if="!item.reviewed"
      ...
      <div v-if="total > limit" class="flex justify-center gap-2 mt-6">
        <button
          @click="loadMistakes(offset - limit)"
          :disabled="offset === 0"
          class="px-3 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40 transition"
        >
          上一页
        </button>
...
import { listMistakes, getMistakeStats, reviewMistake, batchReviewMistakes, deleteMistake, retryMistakes, consolidatePractice } from '../api/client'
...
const weakAreas = ref([])
const consolidateTitle = ref('巩固强化练习')

// ========== 计算属性 ==========
const filterTabs = computed(() => [
  { label: '全部', value: 'all', count: stats.total },
  { label: '待复习', value: 'unreviewed', count: stats.unreviewed },
  { label: '已复习', value: 'reviewed', count: stats.reviewed },
])
...
const retryOptions = computed(() => {
  ...
  try { return JSON.parse(retryQuiz.value.options) } catch { return [] }
})
...
const cOptions = computed(() => {
  ...
  try { return JSON.parse(cQuiz.value.options) } catch { return [] }
})

// ========== 方法 ==========
async function loadMistakes(newOffset = 0) {
  loading.value = true
  offset.value = newOffset
  try {
    const res = await listMistakes({ limit, offset: newOffset })
...
async function loadStats() {
  try {
    ...
    stats.reviewed = res.data.reviewed || 0
    stats.unreviewed = res.data.unreviewed || 0
  } catch {}
}
...
async function autoReviewMistake() {
  ...
  if (item?.mistake_id) {
    try { await reviewMistake(item.mistake_id) } catch {}
  }
}
...
function nextRetryQuestion() {
  if (retryIndex.value + 1 >= retryQuizzes.value.length) {
    ...
    toast.success(`重练完成！正确率 ${retryQuizzes.value.length > 0 ? Math.round((retryCorrect.value / retryQuizzes.value.length) * 100) : 0}%`)
    return
  }
  retryIndex.value++
  resetRetryAnswer()
...
function nextConsolidateQuestion() {
  if (consolidateIndex.value + 1 >= consolidateQuizzes.value.length) {
    ...
    const pct = consolidateQuizzes.value.length > 0 ? Math.round((consolidateCorrectCount.value / consolidateQuizzes.value.length) * 100) : 0
