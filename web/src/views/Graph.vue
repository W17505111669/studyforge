  ...
  <div class="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex items-center gap-3">
        <!-- 布局切换 -->
        <div v-if="selectedMaterial" class="inline-flex rounded-lg border border-gray-200 dark:border-gray-600 overflow-hidden">
          <button
          ...
          >
            <span class="inline-flex items-center gap-1">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v3m0 12v3M5.636 5.636l2.121 2.121m8.486 8.486l2.121 2.121M3 12h3m12 0h3M5.636 18.364l2.121-2.121m8.486-8.486l2.121-2.121"/><circle cx="12" cy="12" r="3" stroke-width="2"/></svg>
              脑图
            </span>
          </button>
        </div>
        ...
        <button
          v-if="isAllMaterials"
          @click="crossMaterialHighlight = !crossMaterialHighlight; updateChart()"
          class="px-3 py-2 rounded-lg border text-sm transition flex items-center gap-1.5"
          :class="crossMaterialHighlight ? 'bg-purple-50 border-purple-300 text-purple-700 dark:bg-purple-900/20 dark:border-purple-700 dark:text-purple-400' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-purple-300 hover:text-purple-600 dark:hover:border-purple-500 dark:hover:text-purple-400'"
          title="高亮显示连接不同材料的边"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- 空状态 -->
    <div v-if="!selectedMaterial" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-16 text-center">
    ...
    <!-- 图谱容器 -->
    <div v-else class="relative bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      <!-- 搜索框浮层 -->
      <div class="absolute top-3 left-3 z-10 flex items-center gap-2">
        ...
        <span v-if="searchQuery && matchCount >= 0" class="text-xs text-gray-400 dark:text-gray-500 bg-white/80 dark:bg-gray-800/80 px-2 py-1 rounded">
          {{ matchCount }} 个匹配
        </span>
      </div>
    ...
    <!-- 节点详情 -->
    <div v-if="selectedNode" class="mt-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 animate-fade-in-up">
...
function getCategoryColor(node) {
  return isAllMaterials.value
    ? (node.material_color || '#6366f1')
...
function computeHierarchicalLayout(nodes, edges, width, height) {
  ...
  if (roots.length === 0) {
    ...
    roots = [nodes.reduce((best, n) => (degree.get(n.name) || 0) > (degree.get(best) || 0) ? n.name : best, nodes[0].name)]
  }
...
function computeMindmapLayout(nodes, edges, width, height) {
  ...
  if (coreNodes.length > 0) {
    root = coreNodes.reduce((best, n) =>
      (adj.get(n.name)?.length || 0) > (adj.get(best)?.length || 0) ? n.name : best,
      coreNodes[0].name
    )
  } else {
...
function initChart(data) {
  ...
  const edges = visibleEdges.map(e => buildEdgeConfig(
  ...
  const option = {
    tooltip: {
      ...
      formatter: (p) => {
        ...
        if (p.dataType === 'edge') {
          const label = p.data._labelText ? ` <span style="color:#6366f1;font-weight:600">${p.data._labelText}</span>` : ''
...
function applySearchHighlight(query) {
  ...
  const updatedEdges = series.links.map(e => buildEdgeConfig(e, {
