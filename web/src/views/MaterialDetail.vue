<template>
  <div class="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
    ...
    <div class="flex items-start gap-3 mb-6">
      <button @click="$router.back()" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-500 dark:text-gray-400 shrink-0">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
        </svg>
      </button>
      <div class="flex-1 min-w-0">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">{{ material?.title || '加载中...' }}</h1>
      ...
      <div class="flex items-center gap-2 shrink-0 flex-wrap justify-end">
        ...
        <button
          ...
          :class="material?.status === 'analyzing'
    ...
    <!-- 错误状态 -->
    <div v-else-if="error" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-red-100 dark:border-red-800 p-12 text-center">
      <p class="text-4xl mb-4">😵</p>
      <h3 class="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">{{ error }}</h3>
      <button @click="loadDetail" class="text-primary-600 hover:underline text-sm">重试</button>
    </div>

    <template v-else-if="material">
      <!-- 分析摘要 -->
      <div v-if="analysis" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
        <div class="flex items-center gap-2 mb-4">
          <span class="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="analysis.key_points?.length" class="mb-5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            >
              <span class="w-5 h-5 rounded-full bg-primary-100 text-primary-700 text-xs flex items-center justify-center font-medium mt-0.5 shrink-0">
                {{ idx + 1 }}
              </span>
              <span class="text-sm text-gray-700 dark:text-gray-300" v-html="renderMath(typeof point === 'string' ? point : point.title || point.name || JSON.stringify(point))"></span>
            </div>
          </div>
        </div>
        ...
        <div v-if="analysis.relationships?.length">
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <span v-if="rel.label || rel.relation" class="text-xs text-purple-500 ml-1">({{ rel.label || rel.relation }})</span>
            </div>
          </div>
        </div>

        <!-- 重要性 -->
        <div v-if="analysis.importance" class="mt-5 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
      ...
      <!-- 无分析数据 -->
      <div v-else-if="material.status === 'completed'" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 mb-6 text-center">
      ...
      <div v-if="material.cards?.length" class="mb-6">
        <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
          <span class="w-8 h-8 rounded-lg bg-green-50 dark:bg-green-900/30 flex items-center justify-center text-green-600 dark:text-green-400">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <div class="flex flex-wrap gap-1">
              <span v-for="tag in parseTags(card.tags)" :key="tag"
                class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded text-xs">{{ tag }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="material.quizzes?.length">
        <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
          <span class="w-8 h-8 rounded-lg bg-orange-50 dark:bg-orange-900/30 flex items-center justify-center text-orange-600 dark:text-orange-400">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-start gap-3">
              <span class="w-7 h-7 rounded-full bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 text-sm flex items-center justify-center font-medium shrink-0">
                {{ idx + 1 }}
              </span>
              <div class="flex-1 min-w-0">
                ...
                <p v-if="quiz.explanation" class="text-xs text-gray-400 dark:text-gray-500 mt-2">解析: <span v-html="renderMath(quiz.explanation)"></span></p>
              </div>
            </div>
          </div>
        </div>
...
const statusLabel = computed(() => {
  return { pending: '待分析', analyzing: '分析中', completed: '已完成', failed: '失败', partial: '部分完成' }[material.value?.status] || material.value?.status || ''
})
