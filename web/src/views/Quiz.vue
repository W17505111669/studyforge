  ...
  <div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
    <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm sm:text-base">AI 根据你的学习材料生成的练习题，即时检验掌握程度</p>
      </div>
      <!-- 报告/答题 切换按钮 -->
      <div v-if="answeredCount === filteredQuizzes.length && filteredQuizzes.length > 0" class="flex items-center gap-2 shrink-0">
        <button
          @click="showReport = false"
          :class="!showReport ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'"
    ...
    <div v-if="showReport && answeredCount === filteredQuizzes.length">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 text-center dark:bg-gray-800 dark:border-gray-700">
          <p class="text-3xl font-bold" :class="accuracy >= 80 ? 'text-green-600' : accuracy >= 60 ? 'text-amber-600' : 'text-red-600'">
            {{ accuracy }}%
          </p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 dark:bg-gray-800 dark:border-gray-700">
          ...
          <div class="space-y-3">
            <div v-for="(tip, idx) in reviewSuggestions" :key="idx"
            ...
            >
              <span class="flex-shrink-0 w-6 h-6 rounded-full bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-amber-600 dark:text-amber-400 text-sm font-medium">
                {{ idx + 1 }}
              </span>
              <p class="text-sm text-amber-800 dark:text-amber-300 leading-relaxed">{{ tip }}</p>
            </div>
            <div v-if="reviewSuggestions.length === 0"
          ...
          <div class="mt-6 space-y-2">
            ...
            <div v-for="stat in dimensionStats" :key="stat.label" class="flex items-center gap-3">
              <span class="text-xs text-gray-600 dark:text-gray-400 w-16 flex-shrink-0 text-right">{{ stat.label }}</span>
              ...
              <span class="text-xs font-medium w-10" :class="stat.rate >= 80 ? 'text-green-600' : stat.rate >= 60 ? 'text-amber-600' : 'text-red-600'">
                {{ stat.rate }}%
              </span>
            </div>
          </div>
          ...
          >
            <div class="flex items-start gap-4">
              ...
              <div class="flex-shrink-0 flex flex-col items-center gap-1">
                ...
                <svg v-if="q._answerHistory?.isCorrect" class="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-2 flex-wrap">
                  <span class="px-2 py-0.5 bg-primary-50 text-primary-600 rounded text-xs font-medium dark:bg-primary-900/20">
                  ...
                  <span v-if="q._answerHistory?.hintsUsed > 0"
                ...
                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-3 leading-relaxed" v-html="renderMath(q.question)"></p>

                <!-- 答题对比 -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                  <div class="p-2.5 rounded-lg text-sm" :class="q._answerHistory?.isCorrect ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'">
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-0.5">你的答案</p>
                    <p :class="q._answerHistory?.isCorrect ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'" class="font-medium" v-html="renderMath(q._answerHistory?.answer || '-')">
                    </p>
                  </div>
                ...
                <div v-if="q.explanation" class="p-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-sm">
                  ...
                  <p class="text-blue-700 dark:text-blue-300 text-xs leading-relaxed" v-html="renderMath(q.explanation)"></p>
                </div>
              </div>
            </div>
          </div>
    ...
    <!-- 智能推荐难度信息横幅 -->
    <div v-if="studyMode === 'recommended' && difficultyInfo"
    ...
    >
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3 text-sm">
          <div class="text-center">
            <p class="font-bold text-lg" :class="difficultyInfo.accuracy >= 80 ? 'text-green-600' : difficultyInfo.accuracy >= 50 ? 'text-amber-600' : 'text-red-500'">
      ...
      <!-- 各难度统计条 -->
      <div v-if="difficultyInfo.difficulty_stats && difficultyInfo.difficulty_stats.length > 0" class="mt-3 pt-3 border-t border-primary-100 dark:border-gray-700">
        <div class="flex items-center gap-4 flex-wrap text-xs text-gray-500 dark:text-gray-400">
          <span v-for="ds in difficultyInfo.difficulty_stats" :key="ds.difficulty" class="flex items-center gap-1.5">
            <span class="px-1.5 py-0.5 rounded" :class="diffBadgeClass(ds.difficulty)">{{ diffLabel(ds.difficulty) }}</span>
            <span>{{ ds.correct }}/{{ ds.total }} 正确</span>
            <span class="font-medium" :class="ds.total > 0 ? (ds.correct/ds.total >= 0.8 ? 'text-green-600' : ds.correct/ds.total >= 0.5 ? 'text-amber-600' : 'text-red-500') : ''">
    ...
    <!-- 进度条 + 题目导航 -->
    <div v-if="filteredQuizzes.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6 dark:bg-gray-800 dark:border-gray-700">
    ...
    <!-- 空状态 -->
    <div v-if="filteredQuizzes.length === 0 && !loading" class="bg-white rounded-xl shadow-sm border border-gray-100 p-16 text-center dark:bg-gray-800 dark:border-gray-700">
      ...
      <div class="flex items-center justify-center gap-3">
        <button v-if="studyMode === 'recommended'" @click="switchMode('all')"
    ...
    <!-- 当前题目 -->
    <div v-if="filteredQuizzes.length > 0 && currentQuiz" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 dark:bg-gray-800 dark:border-gray-700">
      <!-- 题型标签 -->
      <div class="flex items-center gap-2 mb-4">
        ...
        <template v-if="quizMaterialTagMap[currentQuiz.material_id]?.length">
          <span v-for="mt in quizMaterialTagMap[currentQuiz.material_id]" :key="'mt-'+mt"
      ...
      <!-- 题目 -->
      <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6 leading-relaxed" v-html="renderMath(currentQuiz.question)">
      </h2>
      ...
      <div v-if="currentQuiz._answerHistory" class="space-y-4">
        ...
        <!-- 答题结果 -->
        <div class="p-4 rounded-lg" :class="currentQuiz._answerHistory.isCorrect ? 'bg-green-50 border border-green-200 dark:bg-green-900/20 dark:border-green-700' : 'bg-red-50 border border-red-200 dark:bg-red-900/20 dark:border-red-700'">
          <p class="font-medium mb-1" :class="currentQuiz._answerHistory.isCorrect ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'">
          ...
          <p class="text-sm" :class="currentQuiz._answerHistory.isCorrect ? 'text-green-600' : 'text-red-600'">
            正确答案：<span v-html="renderMath(currentQuiz.answer)"></span>
          </p>
          <!-- 使用提示数量标记 -->
          <p v-if="currentQuiz._answerHistory.hintsUsed > 0"
        ...
        <!-- 答错时服务端返回的提示 -->
        <div v-if="currentQuiz._answerHistory.hintFromResponse && !currentQuiz._answerHistory.isCorrect"
        ...
        >
          ...
          <p class="text-sm text-amber-800 dark:text-amber-300" v-html="renderMath(currentQuiz._answerHistory.hintFromResponse)"></p>
        </div>

        <!-- 解析 -->
        <div v-if="currentQuiz.explanation" class="p-4 bg-blue-50 rounded-lg border border-blue-100 dark:bg-blue-900/20">
      ...
      <div v-else>
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            class="px-4 py-3 text-sm font-medium rounded-lg border-2 border-dashed transition-all flex items-center gap-2 disabled:opacity-50
              border-amber-300 text-amber-700 hover:border-amber-400 hover:bg-amber-50
              dark:border-amber-700 dark:text-amber-400 dark:hover:border-amber-600 dark:hover:bg-amber-900/20"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
          ...
          <!-- 已使用提示数标记 -->
          <span v-if="currentQuiz._hintLevel > 0"
            class="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"
          >
            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
        ...
        <div v-if="currentQuiz._hintLevel > 0" class="mt-4 space-y-2">
          <!-- 第1级：概念方向 -->
          <div v-if="currentQuiz._hints.hint_1"
            class="p-3 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800/40 dark:bg-amber-900/20"
          >
            <p class="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1 flex items-center gap-1.5">
              <span class="inline-flex w-5 h-5 items-center justify-center rounded-full bg-amber-200 dark:bg-amber-800/40 text-amber-800 dark:text-amber-300 text-xs font-bold">1</span>
              方向提示
            </p>
            <p class="text-sm text-amber-800 dark:text-amber-300 leading-relaxed" v-html="renderMath(currentQuiz._hints.hint_1)"></p>
          </div>

          <!-- 第2级：关键线索 -->
          <div v-if="currentQuiz._hintLevel >= 2 && currentQuiz._hints.hint_2"
          ...
          >
            <p class="text-xs font-semibold text-orange-700 dark:text-orange-400 mb-1 flex items-center gap-1.5">
              <span class="inline-flex w-5 h-5 items-center justify-center rounded-full bg-orange-200 dark:bg-orange-800/40 text-orange-800 dark:text-orange-300 text-xs font-bold">2</span>
              关键线索
            </p>
            <p class="text-sm text-orange-800 dark:text-orange-300 leading-relaxed" v-html="renderMath(currentQuiz._hints.hint_2)"></p>
          </div>

          <!-- 第3级：接近答案 -->
          <div v-if="currentQuiz._hintLevel >= 3 && currentQuiz._hints.hint_3"
          ...
          >
            <p class="text-xs font-semibold text-red-700 dark:text-red-400 mb-1 flex items-center gap-1.5">
              <span class="inline-flex w-5 h-5 items-center justify-center rounded-full bg-red-200 dark:bg-red-800/40 text-red-800 dark:text-red-300 text-xs font-bold">3</span>
              接近答案
            </p>
            <p class="text-sm text-red-800 dark:text-red-300 leading-relaxed" v-html="renderMath(currentQuiz._hints.hint_3)"></p>
          </div>

          <!-- 某级别没有提示时的占位 -->
          <div v-if="currentQuiz._hintLevel >= 1 && !currentQuiz._hints.hint_1"
      ...
      <div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
        ...
        <!-- 全部完成 -->
        <div v-if="answeredCount === filteredQuizzes.length" class="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
          <div class="p-3 bg-green-50 rounded-lg border border-green-200 text-center dark:bg-green-900/20 dark:border-green-700">
...
const totalHintsUsed = computed(() => filteredQuizzes.value.reduce((sum, q) => sum + (q._answerHistory?.hintsUsed || 0), 0))
...
const totalWrong = computed(() => filteredQuizzes.value.filter(q => q._answerHistory && !q._answerHistory.isCorrect).length)
...
async function submitAnswer() {
  ...
  // 计算用时
  const timeSpent = currentQuiz.value._startTime
    ? Math.floor(Date.now() / 1000) - currentQuiz.value._startTime
    : 0
