<template>
  <div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
    ...
    <div v-else-if="empty" class="text-center py-16">
      ...
      <router-link to="/upload" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium">
    ...
    <div v-else-if="error" class="text-center py-16">
      ...
      <button @click="generatePath(true)" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium">
        重试
      </button>
    </div>
    ...
    <div v-else-if="steps.length > 0">
      <!-- Overview Card -->
      <div class="bg-gradient-to-r from-primary-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 border border-primary-100 dark:border-gray-700 rounded-xl p-4 sm:p-5 mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mb-3">
          ...
          <div class="flex flex-wrap gap-3 sm:gap-4 text-sm">
            <span class="text-gray-600 dark:text-gray-400">
              <strong class="text-gray-900 dark:text-white">{{ steps.length }}</strong> 个学习步骤
            </span>
            <span class="text-gray-600 dark:text-gray-400">
              <strong class="text-gray-900 dark:text-white">{{ totalMinutes }}</strong> 分钟
            </span>
            <span class="text-gray-600 dark:text-gray-400">
              <strong class="text-gray-900 dark:text-white">{{ materialCount }}</strong> 份材料
            </span>
          </div>
        </div>
      ...
      <div class="relative">
        <div
          ...
          :key="idx"
          class="flex gap-3 sm:gap-4 mb-2 last:mb-0"
        >
          <!-- Timeline Rail -->
          ...
          <div class="flex-1 pb-6 last:pb-0">
            ...
            >
              ...
              <div class="p-4 sm:p-5">
                ...
                <p v-if="expandedStep !== idx" class="text-sm text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-2">
                  {{ step.description }}
                </p>
              </div>

              <!-- Expanded Details -->
              <div v-if="expandedStep === idx" class="px-4 sm:px-5 pb-4 sm:pb-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                ...
                <div v-if="step.material_titles && step.material_titles.length > 0">
                  <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">关联材料</h4>
                  <div class="space-y-2">
                    <router-link
                    ...
                    >
                      ...
                      <span class="text-gray-700 dark:text-gray-300 truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{{ title }}</span>
                      <svg class="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 ml-auto shrink-0 group-hover:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        <div class="w-12 h-12 mx-auto mb-3 bg-green-100 dark:bg-green-900/40 rounded-full flex items-center justify-center">
      ...
      <div v-else-if="steps.length > 0" class="mt-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-5">
